...
export class JoinProject implements IJoinProject{
    ...
    async execute(userId: string, requestId: string,teamId:string): Promise<response> {
        try {
            await this.repository.addTeamMembers(new ObjectId(userId),new ObjectId(teamId));
